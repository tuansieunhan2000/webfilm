...
class layoutController {
        ...
        Product.find({ detetedAt: null })
            .then((products) => {
                ...
                res.render("layouts/Products", {
                    products: multipleMongooseToObject(product_arr),
                    countArrCate,
                    nameArrCate,
                });
            })
    ...
    get_all_products_sort_by_increase(req, res, next) {
    ...
    get_all_products_sort_by_decrease(req, res, next) {
        Product.find({ detetedAt: null })
            .sort({ vn_name: 1 })
            .then((products) => {
                let countArrCate = [],
    ...
    show_detail(req, res, next) {
        ...
        Product.findOne({ _id: req.query.key })
            .then((products) => {
                product = products;
            })
            // product.category
            .then(() => {
                Product.find({ detetedAt: null, "category.id": product.category.id })
                    ...
                    .catch((next) => {
                        console.log(
                            'err: Product.find({"category.id": product.category.id}) -> show_detail(req,res,next){'
                        );
                    ...
                    .then(() => {
                        Comment.find({ detetedAt: null, id_product: product._id })
                            ...
                            .catch((next) => {
                                console.log(
                                    'err: Product.find({"comment.id_product": product._id}) ->show_detail(req,res,next){'
                                );
                                console.log(next);
                            });
                    });
